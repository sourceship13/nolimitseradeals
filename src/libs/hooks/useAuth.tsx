...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
        ...
        AsyncStorage.getItem('theme')
          .then(savedTheme => {
            if (savedTheme === null) {
              ...
              console.log(
                '📱 System theme changed - auto-updating app theme to:',
                colorScheme,
              );
            }
          })
  ...
  const fetchDeals = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(
          '📦 dealsData type:',
          Array.isArray(dealsData) ? 'array' : typeof dealsData,
        );
        console.log(
          '📦 dealsData keys:',
          dealsData ? Object.keys(dealsData) : 'null',
        );
        ...
        if (Array.isArray(dealsData)) {
          ...
          const hasFullDealData =
            dealsData.deals.length > 0 &&
            (dealsData.deals[0].deal_id ||
              dealsData.deals[0].dealId ||
              dealsData.deals[0].business_name);

          if (hasFullDealData) {
            finalDeals = dealsData.deals;
            console.log(
              '📦 finalDeals from dealsData.deals (full data):',
              finalDeals.length,
            );
          } else if (
            dealsData.userBusinessDeals &&
            Array.isArray(dealsData.userBusinessDeals)
          ) {
            // Use userBusinessDeals if deals array only has search results
            finalDeals = dealsData.userBusinessDeals;
            console.log(
              '📦 finalDeals from userBusinessDeals:',
              finalDeals.length,
            );
          } else {
            finalDeals = dealsData.deals;
            console.log(
              '📦 finalDeals from dealsData.deals (fallback):',
              finalDeals.length,
            );
          }
        ...
        finalDeals.forEach((deal: any) => {
          const catSlug =
            deal.category_slug || deal.category || deal.category_name;
          const catName =
            deal.category_name || deal.category || deal.category_slug;
        ...
        Object.values(categoryMap).forEach((cat: Category) =>
        ...
        if (savedCategories) {
          ...
          extractedCategories.forEach((cat: Category) => {
            mergedCategories[cat.slug] =
              parsedCategories[cat.slug] !== undefined
        ...
        const annotatedDeals = finalDeals.map((deal: any) => {
          const serverIsHearted =
            deal.isHearted !== undefined
              ? !!deal.isHearted
              : deal.is_hearted !== undefined
              ? !!deal.is_hearted
              : false;
        ...
        const derivedHeartedIds = annotatedDeals
          ...
          .filter(Boolean);
        setHeartedDeals(
          derivedHeartedIds.map((id: string) => ({
            deal_id: id,
            id,
            isHearted: true,
          })),
  ...
  const fetchHeartedDeals = async () => {
    try {
      ...
      const ids = (currentDeals || [])
        ...
        .map((d: any) => d.deal_id || d.id)
        .filter(Boolean);
      const heartedDealsArray = ids.map((dealId: string) => ({
        deal_id: dealId,
        id: dealId,
        isHearted: true,
      }));
      ...
      console.error(
        '❌ Error deriving hearted deals from current deals:',
        error,
      );
      setHeartedDeals([]);
      return [];
    ...
    try {
      ...
      if (isAuth) {
        ...
        try {
          ...
          console.error(
            '⚠️ InitializeApp: Failed to fetch deals, but continuing...',
            dealsError,
          );
        }
        ...
        try {
          ...
          console.error(
            '⚠️ InitializeApp: Failed to fetch hearted deals, but continuing...',
            heartedDealsError,
          );
        }
        ...
        if (currentUser) {
          try {
            ...
            console.error(
              '⚠️ InitializeApp: Using cached user data, server refresh failed:',
              error,
            );
          }
        }
        ...
        try {
          ...
          console.error(
            '⚠️ InitializeApp: Failed to fetch deals for guest, but continuing...',
            dealsError,
          );
        }
      }
  ...
  const loadPreferences = async () => {
    try {
      ...
      const showOnboardingAgain = await AsyncStorage.getItem(
        'showOnboardingAgain',
      );

      // Show onboarding if: never seen it OR user wants to see it again
      const shouldShowOnboarding =
        hasSeenOnboarding !== 'true' || showOnboardingAgain === 'true';
    ...
    if (
      ...
      try {
        ...
        if (user) {
          ...
          fetchHeartedDeals().catch((err: any) =>
            console.error('❌ Failed to refresh hearted deals:', err),
          );
        }
      } finally {
  ...
  const isDealHearted = (dealId: string): boolean => {
    return heartedDeals.some(
      deal => deal.deal_id === dealId || deal.id === dealId,
    );
  };

  // Heart a deal
  const heartDeal = async (
    dealId: string,
    dealObject?: any,
  ): Promise<boolean> => {
    if (!user?.id) {
    ...
    try {
      ...
      const alreadyHearted = heartedDeals.some(
        d => d.deal_id === dealId || d.id === dealId,
      );
      if (!alreadyHearted) {
        const dealToAdd = {
          deal_id: dealId,
      ...
      setHeartedDeals(prev =>
        prev.filter(d => d.deal_id !== dealId && d.id !== dealId),
      );
      return false;
    }
  };
  ...
  const login = async (credentials: LoginCredentials) => {
    try {
      ...
      try {
        ...
        console.error(
          '⚠️ Login: Failed to fetch hearted deals, but continuing login...',
          heartedDealsError,
        );
      }
